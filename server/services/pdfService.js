...
function generatePdfReport(submission, res) {
  ...
  // Set response headers so it triggers a download
  ...
  doc.font('Helvetica-Bold').fontSize(20).text('Risk Assessment Request', { align: 'center' });
  doc.moveDown(1);

  doc.font('Helvetica').fontSize(11).text(`Request ID: ${submission.id.substring(0, 8)}`, { align: 'right' });
  doc.font('Helvetica').fontSize(11).text(`Submitted On: ${new Date(submission.timestamp).toLocaleString()}`, { align: 'right' });
  doc.moveDown(2);
  ...
  doc.font('Helvetica').text(submission.hazards || 'No specific hazards were listed.');
  doc.moveDown(0.5);

  generateLabelValue('Preferred Timeframe:', submission.timeframe ? new Date(submission.timeframe).toLocaleDateString() : 'ASAP');
  doc.moveDown(2);
